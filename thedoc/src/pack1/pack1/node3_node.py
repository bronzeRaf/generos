...
class node3_class(Node):
	...
	def __init__(self):
		...
		# c2
		# Qos profile
		qos_profile_c2 = QoSProfile(history = QoSHistoryPolicy.KEEP_ALL, durability = QoSDurabilityPolicy.SYSTEM_DEFAULT, reliability = QoSReliabilityPolicy.RELIABLE, depth =0)
		...
		self.client_c2 = self.create_client(Addtwo, 'service2', qos_profile = qos_profile_c2)
		#_____
	...
	# Clients
	#____________________________________________
	# This is the call function of the client c2. 
	...
	def client_call_c2(self, x , y ):
		# Wait for service
		while not self.client_c2.wait_for_service(timeout_sec=1.0):
			self.get_logger().info('service not available, waiting again...')
		# Create request and fill it with data
		self.request_c2 = Addtwo.Request()
		self.request_c2.x = x
		self.request_c2.y = y
		self.future_c2 = self.client_c2.call_async(self.request_c2)
		# Result after server's response is stored in 
		# self.future_c2.result().b 
		# self.future_c2.result().h 
		# self.future_c2.result().z 
...
# Clients executables
#____________________________________________
# This is the executable for the client c2.
# Run this executable from the root of the workspace using the command:
# $ ros2 run  node3_c2 arg1 arg2 ...
#
